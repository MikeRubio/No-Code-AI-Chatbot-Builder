/*
  # Complete Chatbot Builder Database Schema

  1. New Tables
    - `profiles` - User profile information and subscription details
    - `chatbots` - Chatbot configurations and flow data
    - `conversations` - Chat sessions between users and bots
    - `messages` - Individual messages within conversations
    - `faq_documents` - Uploaded FAQ documents for training
...
CREATE TABLE IF NOT EXISTS faq_entries (
  ...
  embedding_data text, -- JSON string of embedding array for semantic search
  created_at timestamptz DEFAULT now()
);
...
CREATE POLICY "Users can create FAQ entries for their chatbots"
...
CREATE POLICY "Users can update FAQ entries for their chatbots"
...
CREATE POLICY "Users can delete FAQ entries for their chatbots"
...
CREATE POLICY "Users can create analytics for their chatbots"
...
CREATE POLICY "Users can update analytics for their chatbots"
...
CREATE INDEX IF NOT EXISTS idx_chatbots_published ON chatbots(is_published) WHERE is_published = true;
CREATE INDEX IF NOT EXISTS idx_conversations_chatbot_id ON conversations(chatbot_id);
CREATE INDEX IF NOT EXISTS idx_conversations_user_platform ON conversations(user_identifier, platform);
CREATE INDEX IF NOT EXISTS idx_messages_conversation_id ON messages(conversation_id);
CREATE INDEX IF NOT EXISTS idx_messages_created_at ON messages(created_at);
CREATE INDEX IF NOT EXISTS idx_faq_documents_chatbot_id ON faq_documents(chatbot_id);
