/*
  # Create conversation logs and related tables

  1. New Tables
    - `conversation_logs` - Main table for conversation analytics data
    - `conversation_feedback` - User feedback storage
    - `conversation_exports` - Export operation tracking

  2. Functions
    - `calculate_conversation_stats()` - Calculate conversation statistics
    - `anonymize_conversation_data()` - Anonymize personal data
...
DO $$
BEGIN
  -- Add constraints for conversation_logs
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'conversation_logs_outcome_check') THEN
    ALTER TABLE conversation_logs ADD CONSTRAINT conversation_logs_outcome_check 
      CHECK (outcome IS NULL OR outcome IN ('completed', 'abandoned', 'transferred', 'error'));
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'conversation_logs_satisfaction_check') THEN
    ALTER TABLE conversation_logs ADD CONSTRAINT conversation_logs_satisfaction_check 
      CHECK (satisfaction_score IS NULL OR (satisfaction_score >= 1 AND satisfaction_score <= 5));
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'conversation_logs_nps_check') THEN
    ALTER TABLE conversation_logs ADD CONSTRAINT conversation_logs_nps_check 
      CHECK (nps_score IS NULL OR (nps_score >= 0 AND nps_score <= 10));
  END IF;

  -- Add constraints for conversation_feedback
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'conversation_feedback_type_check') THEN
    ALTER TABLE conversation_feedback ADD CONSTRAINT conversation_feedback_type_check 
      CHECK (feedback_type IN ('rating', 'nps', 'survey', 'thumbs', 'custom'));
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'conversation_feedback_rating_check') THEN
    ALTER TABLE conversation_feedback ADD CONSTRAINT conversation_feedback_rating_check 
      CHECK (rating_value IS NULL OR (rating_value >= 1 AND rating_value <= 5));
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'conversation_feedback_nps_check') THEN
    ALTER TABLE conversation_feedback ADD CONSTRAINT conversation_feedback_nps_check 
      CHECK (nps_score IS NULL OR (nps_score >= 0 AND nps_score <= 10));
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'conversation_feedback_sentiment_check') THEN
    ALTER TABLE conversation_feedback ADD CONSTRAINT conversation_feedback_sentiment_check 
      CHECK (sentiment IS NULL OR sentiment IN ('positive', 'neutral', 'negative'));
  END IF;

  -- Add constraints for conversation_exports
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'conversation_exports_type_check') THEN
    ALTER TABLE conversation_exports ADD CONSTRAINT conversation_exports_type_check 
      CHECK (export_type IN ('csv', 'json', 'xlsx'));
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'conversation_exports_status_check') THEN
    ALTER TABLE conversation_exports ADD CONSTRAINT conversation_exports_status_check 
      CHECK (status IN ('pending', 'processing', 'completed', 'failed'));
  END IF;

  -- Add unique constraint for conversation_logs
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'conversation_logs_conversation_id_key') THEN
    ALTER TABLE conversation_logs ADD CONSTRAINT conversation_logs_conversation_id_key 
...
BEGIN
  ...
  INSERT INTO conversation_logs (
    ...
    COALESCE(conv_record.channel_type, conv_record.platform, 'web'),
...
SELECT 
  ...
  c.chatbot_id,
  c.user_identifier,
  COALESCE(c.channel_type, c.platform, 'web'),
  c.started_at,
  c.ended_at,
