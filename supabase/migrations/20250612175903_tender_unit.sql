/*
  # Fix A/B Test Schema and Functions

  1. New Fields
    - Add metadata fields to ab_tests table for better variant tracking
  
  2. Security
    - Enable RLS on ab_test_results
    - Add policies for proper access control
  
  3. Changes
    - Fix existing functions for A/B test assignment and result tracking
*/

-- Add metadata fields to ab_tests table if they don't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'ab_tests' AND column_name = 'variant_a_metadata'
  ) THEN
    ALTER TABLE ab_tests ADD COLUMN variant_a_metadata jsonb DEFAULT '{}'::jsonb;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'ab_tests' AND column_name = 'variant_b_metadata'
  ) THEN
    ALTER TABLE ab_tests ADD COLUMN variant_b_metadata jsonb DEFAULT '{}'::jsonb;
  END IF;
END $$;
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE tablename = 'ab_test_results' AND policyname = 'Users can read A/B test results for their tests'
  ) THEN
    DROP POLICY "Users can read A/B test results for their tests" ON ab_test_results;
  END IF;
  
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'ab_test_results' AND policyname = 'System can insert A/B test results'
  ) THEN
    DROP POLICY "System can insert A/B test results" ON ab_test_results;
  END IF;
  
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'ab_test_results' AND policyname = 'System can update A/B test results'
  ) THEN
    DROP POLICY "System can update A/B test results" ON ab_test_results;
  END IF;
END $$;

-- Add RLS policies for ab_test_results
