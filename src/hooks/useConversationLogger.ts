...
export function useConversationLogger() {
  const logEvent = useCallback(async (eventType: string, data: any) => {
    try {
      ...
      // Get device info
      const deviceInfo = {
        type: /Mobile|Android|iPhone|iPad/.test(userAgent)
          ? "mobile"
          : "desktop",
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/conversation-logger`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Logging failed: ${response.statusText}. ${errorData.details || ""}`
        );
      }
  ...
  const logConversationStart = useCallback(
    async (
      ...
      await logEvent("start", {
        ...
        outcome: null, // Set to null for active conversations
        data: metadata,
      });
    },
    [logEvent]
  ...
  const logConversationEnd = useCallback(
    async (
      conversationId: string,
      chatbotId: string,
      userIdentifier: string,
  ...
  const logGoalAchieved = useCallback(
    async (
      conversationId: string,
      chatbotId: string,
      userIdentifier: string,
  ...
  const logHandoffRequested = useCallback(
    async (
      conversationId: string,
      chatbotId: string,
      userIdentifier: string,
