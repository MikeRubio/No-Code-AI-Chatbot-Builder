...
export class OpenAIService {
  ...
  async generateResponse(
    ...
    try {
      ...
      const contextualPrompt = this.buildSystemPrompt(systemPrompt, nodeContext);
      ...
      const { intent, confidence } = await this.analyzeIntent(userInput, response);
   ...
   */
  async processFAQDocument(content: string, filename: string): Promise<FAQEntry[]> {
    try {
      if (!this.validateApiKey()) {
   ...
   */
  private async analyzeIntent(userInput: string, aiResponse: string): Promise<{intent: string, confidence: number}> {
  ...
  private buildSystemPrompt(customPrompt?: string, nodeContext?: any): string {
    let prompt = customPrompt || 'You are a helpful AI assistant for a business chatbot.';
    ...
    if (nodeContext) {
      ...
      if (nodeContext.userInfo) {
        prompt += `\n\nUser Information: ${JSON.stringify(nodeContext.userInfo)}`;
      }
    }
...
- Stay in character as a business assistant`;
  ...
  private async findSemanticMatch(query: string): Promise<FAQEntry | null> {
    try {
      const faqText = this.faqContext.map(faq => 
        `Q: ${faq.question}\nA: ${faq.answer}`
      ).join('\n\n');
  ...
  private getFallbackResponse(userInput: string): AIResponse {
    ...
    if (input.includes('hello') || input.includes('hi')) {
      ...
      response = 'For pricing information, please contact our sales team or check our website.';
      ...
      response = 'I understand you\'re asking about that. For the most accurate information, I\'d recommend speaking with one of our team members who can provide detailed assistance.';
    }
