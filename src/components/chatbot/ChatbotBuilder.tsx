...
function ChatbotBuilderContent() {
  ...
  const { chatbots, createChatbot, updateChatbot, publishChatbot, isCreating, isUpdating } = useChatbots();
  ...
  const onDrop = useCallback(
    (event: React.DragEvent) => {
      ...
      const nodeTypeDef = nodeTypeDefinitions.find(def => def.type === draggedNodeType);
      if (!nodeTypeDef) return;

      // Check if user can use this node type
      if (nodeTypeDef.requiresPro && profile?.plan === 'free') {
        toast.error('This feature requires a Pro plan. Please upgrade to continue.');
        return;
      }
  ...
  const handleSave = async () => {
    ...
    if (currentChatbot) {
      ...
      if (profile?.plan === 'free' && chatbots.length >= 1) {
        toast.error('Free plan allows only 1 chatbot. Please upgrade to create more.');
        return;
      }
  ...
  const canUseFeature = (feature: string) => {
    ...
    const proFeatures = [
      'ai_response', 'whatsapp', 'faq_upload', 'appointment', 'action',
      'conditional', 'survey', 'file_upload', 'api_webhook', 'human_handoff'
    ];
    ...
    <div className="h-full flex">
      ...
      <div className="w-80 bg-white/70 backdrop-blur-sm border-r border-gray-200 overflow-y-auto">
        <div className="p-6 space-y-6">
          {/* Chatbot Settings */}
          <div>
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Chatbot Settings</h3>
                ...
                <div key={category}>
                  <h4 className="text-sm font-medium text-gray-700 mb-2 capitalize">
                    {category === 'ai' ? 'AI Features' : 
                     category === 'data' ? 'Data Collection' : category}
                  </h4>
                    ...
                    {nodeTypeDefinitions
                      ...
                      .map((nodeType) => (
                        <motion.div
                          ...
                          whileHover={{ scale: canUseFeature(nodeType.type) ? 1.05 : 1 }}
                          whileTap={{ scale: canUseFeature(nodeType.type) ? 0.95 : 1 }}
                          className={`${
                        ...
                        >
                          <Card hover={canUseFeature(nodeType.type)} className="p-3 text-center">
                            <div className={`w-6 h-6 bg-gradient-to-r ${nodeType.color} rounded-lg flex items-center justify-center mx-auto mb-1`}>
                              <nodeType.icon className="w-3 h-3 text-white" />
                            </div>
                            <p className="text-xs font-medium text-gray-900">{nodeType.title}</p>
                            {nodeType.requiresPro && profile?.plan === 'free' && (
                              <p className="text-xs text-orange-600 mt-1">Pro</p>
                            )}
                          </Card>
                        </motion.div>
                      ))}
